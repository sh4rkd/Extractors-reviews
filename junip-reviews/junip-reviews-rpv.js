;async (fnParams, page, extractorsDataObj, { _, Errors }) => {
  ...
  try {
    let dataToFetch = await page.evaluate((config) => {
      let junipStoreKey =
        document
          .querySelector(".junip-store-key[data-store-key]")
          ?.getAttribute("data-store-key") || config.juniStorekey

      let totalReviews =
        Number(
          document
            .querySelector(".junip-product-review-count")
            ?.textContent?.replace(/\,||\./g, "")
            .match(/\d+/)[0] | 0
        ) ||
        Number(
          Array.from(document.querySelectorAll("script"))
            .filter((element) => element.outerHTML.includes("reviewCount"))[0]
            .outerHTML.match(/(?<=reviewCount":)\d+(?=,)/g)[0]
        ) ||
        "" //modificar el selector de total reviews. || Se añadió el path the {{ Number(Array.from(document.querySelectorAll("script")).filter(element => element.outerHTML.includes('reviewCount'))[0].outerHTML.match(/(?<=reviewCount":)\d+(?=,)/g)[0])] }}, en el caso de que la página no esté cargando correctamente y sea necesario sacar el total reviews del script. NOTA: Si se hace esto, es importante validar si se tiene que inyectar el script en una postNavigate
    ...
    let items = await page.evaluate(async (dataToFetch) => {
      let { junipStoreKey, reviewId, totalReviews, quantityReviews, referrer } =
        dataToFetch
      let reviewsToParse = []
      ...
      let dataFirstFetch = await junipFetch(
        `https://api.juniphq.com/v1/product_reviews?include=product%2Ccustomer%2Cstore%2Crespondent%2Csurvey_answers%2Ctik_tok_urls&filter%5Bproduct_remote_ids%5D%5B%5D=${reviewId}&sort%5Bfield%5D=created_at&sort%5Border%5D=desc&page%5Bsize%5D=${quantityReviews}&v=${junipStoreKey}`,
        junipStoreKey,
        referrer
      )
      ...
      if (totalReviews > quantityReviews) {
        ...
        let lastProductId =
          dataFirstFetch.product_reviews.slice(-1)[0].product_id
        let pageAfter = dataFirstFetch.meta.page.after

        while (totalReviews > 0) {
          let lastReviewFetch = await junipFetch(
            `https://api.juniphq.com/v1/product_reviews?include=product%2Ccustomer%2Crespondent%2Csurvey_answers%2Ctik_tok_urls&page%5Bsize%5D=${quantityReviews}&filter%5Bproduct_ids%5D%5B%5D=${lastProductId}&sort%5Bfield%5D=created_at&sort%5Border%5D=desc&page%5Bafter%5D=${pageAfter}&v=${junipStoreKey}`,
            junipStoreKey,
            referrer
          )
          ...
          lastProductId =
            lastReviewFetch.product_reviews.slice(-1)[0].product_id
    ...
    const sumRatingValues = items.reduce(
    ...
    let reviews = {
      overallRating: {
        value: isNaN(Number(averageRating))
          ? ""
