...
let dataToFetch = await page.evaluate(()=>{
  let junipStoreKey = document.querySelector(".junip-store-key")?.getAttribute("data-store-key")
  let totalReviews = Number(document.querySelector(".junip-product-review-count")?.textContent?.replace(/\,||\./g,"").match(/\d+/)[0]|0)
  let reviewId = __st.rid
...
let items = await page.evaluate(async(dataToFetch)=>{
  let {junipStoreKey, reviewId, totalReviews, quantityReviews} = dataToFetch
  let reviewsToParse = []
  const junipFetch = async(link,junipStoreKey) =>{
